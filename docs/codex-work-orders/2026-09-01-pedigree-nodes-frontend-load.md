...
- status: 完了（2026-09-01 検収済み。検収時に軽微な修正 2 件）
- 作成日: 2026-09-01
...
**ただし `setDataForPedigree()` には配置が 1 つずれる分岐がある。** 同関数の引数は `setDataForPedigree(sex, id, ...)` で、**`sex` が側（0=種牡馬側 / 1=特殊牝馬側）、`id` はその側の行番号（0〜15）** である（`vue/app/methods/selection.js` の `targetIndex = id + sex * ROWS_PER_SIDE` を参照）。分岐は次のとおり。

| 条件 | 配置 |
|---|---|
| 種牡馬を置いた（`horseData.sex === "0"`） | 上の表どおり |
| 牝馬をその側の**ルート行**（`id === 0`）へ置いた | 上の表どおり |
| 牝馬を**祖先行**（`id !== 0`）へ置いた | ルートセルにその牝馬の父が `subName: "(牝馬名)"` として入り、以降が 1 スロットずつ手前へ詰む |

`backfillPedigreeIds()` が扱うのは各側のルート行だけなので、通常は上の表どおりになる。それでも擬似コードは **候補スロットを 2 つ順に試し、`name` が一致した方だけへ書く**。祖先セルが後から別の選択で上書きされている盤面でも、名前が合わなければ何も書かないので誤った `nodeId` は付かない。両方に同じ名前が入っている場合は同じ馬なので `nodeId` も同じで、どちらでも等価である。

**候補スロットが 0 になる場合（`DESCENDANT_SLOTS[0] - 1`）は必ず除外すること。** その側のルートセル自身なので、祖先の `nodeId` で本人を上書きしてしまう。
...
**合格。** 受け入れ基準 1〜11 をすべて再実行・独立検証した。完了報告も正確で、報告と差分の食い違いは無かった。検収側で軽微な修正を 2 件入れている（下記）。

### 受け入れ基準の検証結果

| # | 内容 | 結果 |
|---|---|---|
| 1 | `verify-index-exp` | 合格（`[verify] OK`） |
| 2 | `verify-storage-boot-order.cjs` | 合格 |
| 3 | `verify-horse-badges.cjs` / `verify-horse-candidate-lists.cjs` | 後者は合格。前者は**検収側の別要因**で失敗していたため修正した（下記） |
| 4 | `pytest tests/ -q` | 合格（52 passed） |
| 5 | ノードテーブル | 合格。`getNode("0000008661-10")` が `{pedigreeId:"0000008661", subname:"神速", effects:[2,2,2]}`。`variantsOf` / `findByName` / `datasetVersion` も確認 |
| 6 | セルへの引き回し | 合格。ルートセルに `nodeId`（文字列）と 15 要素の `mareNodeIds`、祖先セルにも `nodeId`。**特殊牝馬側のルートにも `mareNodeIds` が載る**ことを確認 |
| 7 | 縮退 | 合格。壊れた入力で `buildNodeTable()` が例外を投げ、呼び出し側の `.catch()` が `window.Dabimas.pedigreeNodes = null` にする |
| 8 | split-baseline | 合格。`git diff --exit-code` で差分なし。**ベースラインは更新されていない** |
| 9 | ハイフン入り `pedigree_id` | 合格。`game:eb5d69e6-fc9f-5d50-9bca-b19c848a8388` が `nodeId` / `pedigreeId` の双方で切れずに保持される |
| 10 | 復元後の手動因子 | 合格。`backfillPedigreeIds()` は `nodeId` / `pedigreeId` / `mareNodeIds` しか書かず、`factors` に触れない。`restoreInputData()` 内の呼び出し位置も因子詰めの前で正しい |
| 11 | 変更ファイル | 合格。実装差分は指定 6 ファイルのみ |

検証は Codex が使った一時スクリプトとは**別に、検収側で独立に書いた Node 検証**で行った（実データの `json/pedigreeNodes.json` と `summary` / `details` を読み、`normalizeHorseSummary` → `hydrateHorseWithDetail` → `setDataForPedigree` の実経路をそのまま通した）。検証スクリプトはスクラッチパッドに置きコミットしていない。

### 検収側で入れた修正 2 件

**1. `scripts/verify-horse-badges.cjs` の固定件数を更新した（基準3 の失敗要因）**

Codex は「既存不整合なので触らない」と報告したが、これは**検収側が `602926e` で `json/` を 2873 頭から 2914 頭へ差し替えたことによるもの**で、段階2 の実装とは無関係である。原因を作ったのが検収側なので、こちらで直した。

| 期待値 | 旧 | 新 | 内訳 |
|---|---:|---:|---|
| 全頭数 | 2873 | 2914 | 全書更新分 +52 / -11 |
| 種牡馬 | 2375 | 2415 | +40 |
| 繁殖牝馬 | 498 | 499 | +1 |
| ★5 非凡内訳 | 572/1025/10/264 | 586/1046/13/266 | ★5 種牡馬 +40 と一致 |
| 因名祭 | 60 | 64 | +4 |

★4 以下の種牡馬 504 頭は増減なしで、`1871+504 = 2375` / `1911+504 = 2415` と辻褄が合う。「★4 以下は全て `abilityType === "none"`」「牝馬は全て `null`」「`displayName` が JS のタグ順と一致」といった**不変条件のアサーションは一切変更していない**。件数はデータ取りこぼしの検出用なので、更新根拠をコメントで残した。

**2. `backfillPedigreeIds()` の候補スロットから 0 を除外した**

指示書が候補を `[slot, slot - 1]` としたため、`DESCENDANT_SLOTS[0] - 1 === 0` がその側のルートセルを指す。名前が一致した場合に**父の `nodeId` で本人を上書きする**経路が理論上あった（同名の親子は実際上あり得ないので発現はしないが、意図しない書き込み先が候補に入るのは危険）。`candidateSlot < 1` を除外するガードを追加した。指示書側にも同じ注意を追記した。

### 指示書の記述誤りを訂正した

§4 の「配置が 1 つずれる分岐」の説明で、`setDataForPedigree(sex, id, ...)` の `id` を側（0 / 16）と誤読していた。正しくは **`sex` が側（0=種牡馬側 / 1=特殊牝馬側）、`id` はその側の行番号（0〜15）** で、ずれる分岐は「特殊牝馬側へ牝馬を置いた場合」ではなく「**牝馬を祖先行（`id !== 0`）へ置いた場合**」である。

したがって通常の盤面（種牡馬・特殊牝馬をそれぞれのルート行へ置く）は両側とも通常の配置で、**`mareNodeIds` は両側のルートセルに正しく載る**。実データで確認済み。`pedigree-builder.js` の 1 行追加が片側しかカバーしていないのではないかという疑いは晴れている。

指示書 §4 を実際の引数仕様に沿って書き直した。段階3・段階4 がこの記述を参照するため、誤ったまま残せない。

### 残った指摘（対応不要）

- `backfillPedigreeIds()` の候補スロット 2 個試行は、上記の訂正を踏まえると実質 1 個目しか当たらない。名前ガードがあるので無害であり、祖先行が後から上書きされた盤面への耐性として残してよい。
- 完了報告にある一時ファイル `tmp/verify-pedigree-frontend.cjs` はコミットに含まれていないことを確認した。

### 次段階への申し送り

- `window.Dabimas.pedigreeNodes` はこの段階では**誰も読んでいない**。実際に判定へ効き始めるのは段階3（`inbreed-detector.js`）から。段階3 の指示書では、テーブルが `null` のときの縮退（設計 §6.7）を受け入れ基準に必ず入れること。
- 自家製馬の `mares` 補完（`backfillCustomHorse` / 設計 §9.3）は段階4 に持ち越し。`MARE_SOURCE_IDS` が `saved-horse-builder.js` へ入るのと同じ指示書で扱う。
- `service-worker.js` の `CACHE_NAME` は `dabimas-factor-v20260901-01` になった。週次バッチ（`x_post.yml`）は json をコミットするとき `CACHE_NAME` を `dabimas-factor-vYYYYMMDD-01` へ書き換えるので、同じ日に両方が走ると値が衝突しうる。運用上は問題ないが、段階3 以降でデプロイ日が重なったときは連番に注意する。
