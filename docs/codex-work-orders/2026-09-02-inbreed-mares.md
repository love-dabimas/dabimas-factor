...
- status: 完了（2026-09-02 検収済み。修正なし）
- 作成日: 2026-09-02
...
**合格。修正なし。** 受け入れ基準 1〜10 を再実行・独立検証した。完了報告も正確で、報告と差分の食い違いは無かった。段階3 の検収で見逃した「`nodeId` を持つ盤面での縮退」も今回は確実に確認した。
...
| 4 | 2 引数呼び出しが段階3 と一致 | **合格**（下記のとおり `nodeId` + `mareNodeIds` を持つ実盤面 3 つで確認） |
| 5 | `mareNodeIds: null` で段階3 と一致 | 合格 |
| 6 | TC-M01〜M07 | 合格。TC-M01 の 15625 を含め、実データで再現 |
| 7 | 対応表の男系での再現 | 合格 |
| 8 | クロス群の増加が実測と整合 | **合格**（独立測定でも再現。下記） |
| 9 | ベースライン盤面 | 合格。変化なしを Node 上で再現して確認 |
| 10 | 変更ファイル | 合格。実装差分は指定 3 ファイルのみ |

### 基準4 を実盤面で確認した（段階3 の教訓）

段階3 の検収では `nodeId` を持たない盤面ばかりで縮退を検証したため不具合を見逃した。今回は `nodeId` と `mareNodeIds` が全セルに入っている実データの盤面で、段階3 の最終コミット（`c3d3a11`）の実装と 2 引数呼び出しを直接突き合わせた。

```text
ヴァーミリアン × サドラーズギャル : 段階3 と完全一致（count 9）
ダッシャーゴーゴー × シル        : 段階3 と完全一致（count 2）
ディープインパクト × エアグルーヴ  : 段階3 と完全一致（count 2）
```

`setDataForPedigree()` がセルへ振る `uuid` は毎回ランダムなので比較対象から除いた。それ以外の既存 6 キーは全一致。`buildMareOccurrences()` が `nodeTable` 不在時に空配列を返す実装（Codex の設計判断）が効いている。

### 基準8 を独立に再測定した

Codex とは別の seed・別のサンプル数で、実装の `judgeInbreed()` をそのまま呼んで測定した。

| | 完了報告（4000 組） | 検収側（1000 組・別 seed） |
|---|---|---|
| `crosses` 平均（牝馬なし → あり） | 0.738 → 1.136 | 0.734 → 1.164 |
| 増分 | +0.398 | +0.430 |
| 1 件以上増加 | 22.6% | 23.0% |

指示書の 1.07 → 1.47 は「master の関係を直接グループ化した値」で、実装の `crosses` は段階3 から引き継いだ祖先・例外除外後の認定ペアを種にするため約 0.31 低い。**増分と増加率はどちらの測り方でも一致しており、牝馬の投入漏れではない。** 完了報告の説明は正しい。

`dangerous` の発生率も測った: 牝馬なし 19/1000 → 牝馬あり 22/1000（2.2%）。

### 受け入れた設計判断

- **`nodeTable` が無いときは牝馬擬似出現を作らない。** 牝馬は名前を持たず master 全兄妹も引けないので、作っても判定できない。基準4 の完全縮退もこれで保証される。
- **新規クロスの種を「少なくとも片方が牝馬出現」のペアに限定した。** 男系×男系は既存の除外後の認定結果を維持し、段階4b の枝除外と混ぜない。妥当。
- **`backfillCustomHorse()` が保存元 config の 2 頭から `setDataForPedigree()` で 32 セルを再構成してから ID を補完する。** セル対応表を複製せず現行の血統展開規則を単一の根拠にしている。段階2 の検収で「配列を 2 か所に置くと必ずずれる」と指摘した方針と一致していて良い。

### 申し送り（段階4b へ）

完了報告の「残課題」にあるとおり、`crosses` の男系側が既存の祖先除外の影響を受けて直接値より約 0.31 低い。段階3 の検収でも同じことを申し送った。**段階6 で `dangerous` が配合理論の「危険な配合」を駆動し始めると、この除外の効き方が判定結果に直結する。** 段階4b（仕様書 §11 の枝除外）で、既存の `addExcludedAncestorPairs` と §11 の枝除外の役割分担を整理すること。

### 実機で確認したい挙動（未決事項2 の判断材料）

検収中に見つけた具体例を記録しておく。**ヴァーミリアン × サドラーズギャル**（サドラーズギャルはヴァーミリアンの父エルコンドルパサーの母）。

```text
牝馬なし（段階3）: count 9 / crosses 4 群 / dangerous = false
牝馬あり（段階4）: count 13 / crosses 8 群 / dangerous = true

新たに立つ 4 群（すべて画面には出ない）
  サドラーズギャル 62.500%  種牡馬側 牝系枠 3代 × 牝馬側 セル16 1代   ← 危険な配合の原因
  Glenveagh       31.250%
  Fairy Bridge    15.625%
  Lisadell        21.875%（Special を全兄妹として取り込み）
```

サドラーズギャルが「特殊牝馬本人（1 代）」と「種牡馬の父の母（3 代）」で 1×3 になり、血量 62.5% で危険な配合の閾値 50% を超える。**設計 §7.1 のとおり牝馬クロスは画面に出さないので、ユーザーにはこの 62.5% がまったく見えない。** 着色セルも `sameNameGroups` も段階3 と同一である。

段階4 の仕様としては正しいが、「ゲームでは危険な配合と出るのにアプリでは何も出ない」状態になる。設計 §14 の未決事項2（血量・危険な配合を画面に出すか）を判断するときの具体例として使えるはず。
