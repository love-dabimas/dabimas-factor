...
### 実機（ブラウザ）での確認（2026-09-08）

`.claude/launch.json` の `static-verify`（ポート8767）でリポジトリ直下を配信し、画面操作で確認した。

**操作手順（UI で実際に到達できる手順）**

1. 左① に ダンスインザダーク（通常版）を選択 → 父側16セルが埋まる
2. 右⑩（localIndex 3 = 母の母の父の枠）に **繁殖牝馬 ダンスパートナー** を選択
   → `deleteHorses` が同じ uuid のセルを消すため、母側の 16/17/18/20/21/24〜27 が null になる
   → 選んだセルは `サンデーサイレンス` / `subName = "(ダンスパートナー)"` / **`placeholderMareNodeId = "0000262374-00"`**
3. 右② に トニービン を選択 → 残りの母側セルが埋まり、**セル16 は「ワタシノヒンバ」で自動補完**されて盤面が完成する

**結果**

| 項目 | 実機の値 | 期待値 |
|---|---|---|
| `inbreedColorIndexes` | `[0]` | `[0]` |
| `dispColor` の着色セル | `[0]` | セル0のみ |
| `crosses` | `ダンスインザダーク 血量75000 [[0,""],[null,"M"]]` | 同左 |
| `dangerous` | `true` | `true` |
| 理論表示 | **危険な配合** | 危険な配合 |

画面上は **ダンスインザダークのセルだけが赤くなり、ダンスパートナー側は光らない**。
牝馬の出現は `index === null` なので既存の表示フィルタで自動的に除外され、
フェーズ2 の「牝馬側の表示を出さない」要件を先取りで満たしている。

**補足: 受け入れ基準1の盤面は合成である**

指示書の基準1は「母側ルート＝エアグルーヴ ＋ 右⑩＝ダンスパートナー」としていたが、
実機ではこの2つは同時に成立しない。祖先セルへ牝馬を選ぶと `onChangeMain` →
`deleteHorses` が同じ uuid のセルを全部消すため、ルートが必ず null になるからである
（`vue/app/methods/selection.js:125`）。
実機では上記のとおりセル16が「ワタシノヒンバ」になるが、**判定結果は完全に同じ**だった。
牝馬枠 "M" は `placeholderMareNodeId` から解決されるため、ルートの nodeId に依存しない。

**注意: ポート8766 に別ディレクトリの古いコピーを配信している http.server が常駐している。**
最初そこへ接続してしまい、`crosses` を返さない古い `inbreed-detector.js` と
`json/pedigreeNodes.json` の 404 を踏んだ。検証には使わないこと。
