...
- status: 完了（2026-09-09 検収済み。パート1基準6の記述誤りは検収記録で訂正）
- 作成日: 2026-09-09
...
**合格。実装側の修正は不要。** 報告された「C01の旧期待値との不整合」は、
**依頼側（Claude）の受け入れ基準6の書き方が誤っていた**もので、実装の不備ではない。

### 依頼側の記述誤りの訂正

基準6に「前フェーズの受入基準を再実行し、**すべて従来どおりの値**になること」と書いたが、
本フェーズは**解決できる牝馬枠を増やす**変更なので、内部クロスが増えるのは当然である。

C01（父側＝ドゥラメンテ／母側ルート＝自家製B、Bの母＝custom A）では、
新たに解決された牝馬枠どうしで **アドマイヤグルーヴ（父側 "M"）× A（母側 "M"）** が
全兄妹として成立する。A の父母はサンデーサイレンス×エアグルーヴで、
アドマイヤグルーヴと完全に一致するので、**これは正しい検出**である。

- 両方とも牝馬（`index === null`）なので**表示は増えない**。着色 `[1,17]` は不変
- `dangerous` も `true` のまま不変
- C01 の本来の要件「**ドゥラメンテ本人**との全兄妹は不成立」は維持されている

正しい基準は「従来どおりの値」ではなく、
「**着色と `dangerous` が不変**であり、増えた内部クロスが新たに解決された牝馬枠で
説明できること」だった。今回は3フェーズ連続で同じ書き方の誤りをしている。

### 検収側で再実行した結果

| 基準 | 結果 |
|---|---|
| 1-1 報告1（保存せず父母を祖先セルへ） | OK。着色 `[1]` / シンボリルドルフ 血量50000 / `dangerous=true` / 牝馬枠 `"M"` が `implied`（父=パーソロン1971・母=スイートルナ） |
| 1-2 報告2（自家製を祖先セルへ） | OK。着色 `[1]` / 血量50000 のみ。スピードシンボリと Palestine の誤検出が消えた |
| 1-3 報告3（自家製2段） | OK。着色 `[1,12,30,14,26]` / シンボリルドルフ 血量28125 / 牝馬枠 `"MMMM"`(5代目) に custom A |
| 1-4 通常盤面の回帰 | OK。**別サンプル2000組**（`abccc16` と比較）で差分0件 |
| 1-5 ★薄め馬 | OK。同名の★薄め馬2セルの間にクロスは出ない |
| 1-6 前フェーズの受入 | OK（上記の訂正付き）。B01 `[1]` / B03 `[2,17]` / C02 `[0]` / E01 血量100000・75000・12500 / F01 血量75000 はすべて不変 |
| 2 工程診断 | OK。`scripts/verify-p2d-plan.cjs` の assert 内容を読んだうえで実行。仮想牝馬の ref が `plan:step:1`、父母が A の父母と一致、画面側の判定と一致、再実行で安定、**元 resolver が書き換えられていない**ことを検査している |
| 3 共有 | OK。`scripts/verify-p2d-sharing.cjs` を同様に確認して実行。再帰収集が `ch_A/ch_B/ch_child/ch_parent` を集め、受け取り側で父母 ref と判定全体が作者側と一致、欠落IDは `null`、復元順が custom→edit→restore |
| 4 resolver 未指定の警告 | OK。実測。nodeTable あり＋resolver なし→1回、nodeTable なし→0回、両方あり→0回 |
| 既存ガード | OK。`node --check` 5件、`verify-index-exp` → `[verify] OK`、`pytest` 52 passed、`git diff --check` 指摘なし、BOM なし・LF |

### 画面確認

ポート8767 で ServiceWorker とキャッシュを消してから、アプリ本来の `onChangeMain` で報告1の盤面を組んだ。

```
cell0=トウカイテイオー央獅 / cell17=アイアンリージ巌瓏 / cell19=パーソロン1971
cell23=スピードシンボリ(スイートルナ) / cell16=ワタシノヒンバ（自動補完）

inbreedColorIndexes = [1]        dispColor = [1]        dangerous = true
crosses = シンボリルドルフ 血量50000 [[1,"F"],[null,"M"]]
牝馬枠 "M" = {kind:"implied", father: パーソロン1971, mother: スイートルナ}
```

Vue の `dispColor` まで期待どおりに反映されている。

### 実装側の良かった判断

- 暗黙の個体を作る条件を「**父と母の両方が既知**」に絞った。依頼側の試作は片方だけでも作っていたが、
  `sameKnownParent` は両親が揃わないと成立しないので、片親だけの個体は決してクロスできない。
  作らないほうが正しい
- `composeVirtualMareBoard` の第3引数を省略したときも `implied` で個体を残すようにした。
  単独利用の呼び出し元があっても情報が落ちない
- 共有復元で、DB書き込み後に custom / edit 両キャッシュの再読込を待ってから `restoreInputData` を呼ぶ。
  `identityResolver` は computed なので、これが無いと復元直後の判定が古いレジストリで走る

### 残課題

- 別端末の実 IndexedDB を使った共有往復のブラウザE2Eは未実施（Node の実コンポーネント検証まで）。
- 依頼側の受け入れ基準の書き方（「差分0」「従来どおり」と書いて不変条件と変化してよい範囲を分けない）は
  3フェーズ連続で誤った。次回以降は**不変条件（着色・dangerous・表示）**と
  **増えてよいもの（内部クロス）**を分けて書く。
