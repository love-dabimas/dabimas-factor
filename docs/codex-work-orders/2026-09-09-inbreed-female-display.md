...
- status: 完了（2026-09-09 検収済み。受け入れ基準3・5の記述誤りは検収記録で訂正）
- 作成日: 2026-09-09
...
**合格。実装側の修正は不要。** 報告された「基準3・5との矛盾」は、**依頼側（Claude）の受け入れ基準の
記述誤り**であり、実装の不備ではない。Codex が基準に合わせて実装を曲げず、差分を明示して
報告した判断は正しい。

### 依頼側の記述誤りの訂正

**基準3の誤り。** 「B03は着色も分類も変わらない」と書いたが、B03のクロスは
パーソロン1971（cell2）× 覇煌（cell17）で、**同一実馬の別 variant** である。
基準4が「同一実馬の別 variant は同一馬に分類される」と要求している以上、
B03の分類が `siblingGroups` から `sameNameGroups` へ移るのは**必然**である。
基準3と基準4は互いに矛盾していた。正しくは「B03は**着色**が変わらない」。

**基準5の誤り。** 「着色差分は0件」「分類差分はすべて全兄妹→同一馬の一方向」と書いたが、
これは依頼側が1サンプル（1000組）で観測した結果を一般化したものだった。
サンプルを変えると両方とも成り立たない。正しい条件は
「**内部結果（count・dangerous・crosses・血量・出現・自己祖先警告・至高）の差分が0**であり、
着色差分は『同一馬の相手を持たない牝馬の抑制』だけで説明できること」である。

### 検収側で再実行した結果

依頼側が独立に書いた検証スクリプトで再実行した。基準5は**別サンプル（seed `0x13572468`、5000組）**
に広げ、差分の性質まで機械的に検査した。

| 基準 | 結果 |
|---|---|
| 1. A01 | OK。着色 `[0,16]`→`[0]`、`siblingGroups` `[[0]]`、血量100000・`dangerous=true`・`count=2` 不変、内部出現は 0/16 のまま |
| 2. B01・C02 | OK。`[1,16]`→`[1]`、`[0,16]`→`[0]`。血量75000／100000 不変 |
| 3. B03・C01 の**着色** | OK。`[2,17]`／`[1,17]` のまま。分類変更は上記のとおり基準側の誤り |
| 4. F01 | OK。`sameNameGroups` に `[0,17]` |
| 5. 回帰（5000組・別サンプル） | OK。**内部結果の差分0件**。着色差分1件で、すべて「同一馬の相手を持たない牝馬の抑制」で説明できる（追加された着色は0件）。分類差分82件（全兄妹→同一馬54／同一馬→全兄妹27／同数1） |
| 6. D05 | OK。基準5で着色の追加が0件、母側セルを含む着色が維持されている |
| 7. 既存ガード | OK。`verify-index-exp` → `[verify] OK`、`pytest` 52 passed、`node --check`、`git diff --check` 指摘なし、BOM なし・LF |
| 8. 画面 | 実装側の報告で確認（SW とキャッシュを消したうえで父側のみ着色・theory_08 維持・コンソールエラー0件） |

### マージ前の追加確認

`origin/main` の最新データ（feature より新しい。ノード10,279件／血統8,869件）を取り出して
ケース検証を再実行し、**B01／B03／C01／C02／E01／F01 のすべてが同じ結果**になることを確認した。
検証で参照するノード・血統行に差分は無い。

さらに、**マージベース `5f2f16c` から `HEAD` までの通算の影響**を、main の最新データと
実際の例外ルールで測定した。マスタ馬だけの盤面3000組で、
**着色・`dangerous`・`count` のいずれも差分0件**。
（別サンプル5000組では着色差分1件。全兄妹の牝馬ルートが光らなくなる想定どおりの変化）

### 気づき

- 受け入れ基準に「差分0」と書くときは、1サンプルの観測を一般化しないこと。
  正しくは不変条件（内部結果）と、変化してよい範囲（差分の性質）を分けて書く。今回はそこを誤った。
- 工程診断の一時 registry、共有の再帰収集、`resolver` 未指定時の扱いはフェーズ2dに残る。
