...
代わりに**表示との乖離が残る**。ヴァーミリアン × サドラーズギャルでは、サドラーズウェルズが index 5 / 17 で着色されたまま `crosses` には無い。着色は牝馬枠を知らない既存パイプラインが作っているためで、指示書 §5 で許容した範囲どおり。段階4c（表示を `crosses` 由来へ寄せる）で解消すること。**着色が目に見えて減る変更になる**ので、単独リリース + ベースライン更新が必要。
