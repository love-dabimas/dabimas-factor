...
**合格。修正なし。** 受け入れ基準 1〜11 を再実行・独立検証した。完了報告も正確で、報告と差分の食い違いは無かった。
...
| 4 | `nodeTable` なしで段階4 と完全一致 | 合格。`nodeId` + `mareNodeIds` を持つ実盤面 3 つで、`crosses` / `dangerous` を含む**全 8 キー**が一致 |
| 5 | 表示 3 集合が不変 | 合格。実データ 30 盤面で `sameNameGroups` / `siblingGroups` / `inbreedColorIndexes` が全件一致 |
| 6・7 | path 網羅性と枝親の解決 | 合格。実装は `byPath.get(path.slice(0,-1))?.nodeId ?? null`、ルートは `null` |
| 8 | ヴァーミリアン × サドラーズギャル | **合格。指示書の予測と 1 件も違わなかった**（下記） |
| 9 | クロス群の減少 | 合格。別サンプルでも再現（下記） |
| 10 | `dangerous` の変化 | 合格。変化なしを確認 |
| 11 | 変更ファイル | 合格。`DESCENDANT_SLOTS` の一元化を含め 3 ファイル |

### 基準8 は指示書の予測どおりだった

```text
crosses 8 群 → 4 群 / dangerous = true（維持）

残った 4 群
  サドラーズギャル   62.500%  出現2   ← 1×3。危険な配合の原因
  Lisadell          21.875%  出現3   ← 別枝（種FMMM × 牝FMM）
  ノーザンダンサー    18.750%  出現3   ← 別枝（種MMFF × 牝FF）
  ヘイルトゥリーズン   6.250%  出現2
```

指示書 §背景の「具体例」で採用と予測した 4 件がそのまま残り、除外と予測した 6 件（サドラーズウェルズ / Glenveagh / Fairy Bridge / Lisadell(種FMMM × 牝MM) / シアトルスルー / ノーザンダンサー(種FMFF × 牝FF)）がすべて消えた。**サドラーズギャル本人のクロスが立っているので、その配下の祖先クロスは重複として消え、別枝から合流したものだけが残る**という §11 の狙いどおりの結果である。

### 基準9・10 を独立に再測定した

| | 完了報告（4000 組・seed 0x20260902） | 検収側（1000 組・別 seed） |
|---|---|---|
| `crosses` 平均 | 1.13625 → 0.7825 | 1.0980 → 0.7630 |
| 減少幅 | −0.354 | −0.335 |
| 群が減った盤面 | 20.2% | 20.2% |
| `dangerous` | 82 → 82（変化なし） | 18 → 18（変化なし） |

標本が違うので絶対値は少しずれるが、減少幅も減少盤面率も一致した。指示値 0.81 との 0.03 程度の差について完了報告が「同じ標本での段階4 の直接グループ化平均も指示書より 0.0275 低く、差がそのまま標本差」と説明しているのは妥当である。

完了報告にある「減少数分布の `-1:3`」（3 盤面でクロス群が 1 増えた）は、既存の祖先除外から切り離したことで legacy 側が過剰に消していた群が復活したもので、意図した挙動。検収側の 1000 組標本では増えた盤面は 0 だった。

### 受け入れた設計判断

- **`DESCENDANT_SLOTS` の定義元を `pedigree-builder.js` にした。** 読み込み順が `inbreed-detector.js` / `bootstrap.js` のどちらより先なので、順序を変えずに両方から参照できる。段階2 の検収で「同じ配列が 2 か所にあると必ずずれる」と指摘した方針の完遂で、`bootstrap.js` の複製も消えた。
- **`nodeTable` 不在時は path・枝親を持つ 31 ノード判定自体を使わず、段階4 の経路をそのまま残した。** 基準4 の完全一致がこれで保証される。occurrence の追加フィールドも `nodeTable` がある場合だけ付ける徹底ぶり。
- **§11 で除外したペアでも、群が別経路で成立した場合は血量計算用に出現として取り込む。** 仕様書 §12.1「同一クロスグループの全出現位置を加算」に忠実。Lisadell 群の出現が 3 件なのはこのため。

### 申し送り（段階4c へ）

段階3 の検収から持ち越していた「`crosses` が既存の祖先除外の影響を受ける」という懸念は**本段階で解消した**。`crosses` は 31×31 の直接突き合わせ + §11 だけで決まるようになり、legacy 側から独立した。

代わりに**表示との乖離が残る**。ヴァーミリアン × サドラーズギャルでは、サドラーズウェルズが index 5 / 17 で着色されたまま `crosses` には無い。着色は牝馬枠を知らない既存パイプラインが作っているためで、指示書 §5 で許容した範囲どおり。段階4c（表示を `crosses` 由来へ寄せる）で解消すること。

**［2026-09-03 追記］この検収記録で「着色が目に見えて減る変更になる」と書いたのは誤りだった。** 段階4c の指示書を書くときに実測したところ、着色セル数の平均は 1.591 → 1.593 でほぼ変わらず、着色が変わる盤面は 1500 組中 34 組（2.3%）だけだった。§11 が消すクロスの多くは旧パイプラインの `addExcludedAncestorPairs` も消していたためである。正しい数値は `docs/codex-work-orders/2026-09-03-inbreed-display-convergence.md` を参照。
