...
依頼側が独立に書いた検証スクリプトで全基準を再実行した。基準1の比較対象は `HEAD~1`（フェーズ2a）。

| 基準 | 結果 |
|---|---|
| 1. マスタ盤面が変わらない | OK。seed `0x20260909` の1000組で差分0 |
| 2. ケース2（B01・B03） | OK。B01=着色`[1,16]`／シンボリルドルフ 血量75000／出現`[[1,"F"],[16,""]]`／`dangerous=true`。B03=着色`[2,17]`／パーソロン37500・スイートルナ37500／ルドルフ無し／`dangerous=false` |
| 3. ケース3（C01・C02） | OK。C01=着色`[1,17]`／キンカメ50000のみ。C02=着色`[0,16]`／ドゥラメンテ100000 |
| 4. 血量（E01） | OK。1×1=100000／1×2=75000／3×3=12500。3×3 では `ch_X` 群が50000未満 |
| 5. 自家製だけの盤面 | OK。クロス1件（血量100000）。修正前は0件だった |
| 6. 自家製牝馬を祖先セルへ（A05） | OK。ディープインパクト 血量75000／出現`[[0,""],[null,"M"]]`／着色に `0` を含む |
| 7. エディット種牡馬（D08） | OK。ディープ 血量75000／出現`[[0,""],[17,"F"]]`。親キーは `edit:edit_1` でベース馬と不一致 |
| 8. 偽陽性（D07・F01） | OK。同名★薄め馬に偽クロス無し。パーソロン1971×覇煌のクロス（血量75000）は維持 |
| 9. 既存ガード | OK。`node --check`、`verify-index-exp` → `[verify] OK`、`pytest` → 52 passed、`git diff --check` 指摘なし、BOM なし・LF |
| 10. 画面 | OK。下記 |

### 検収側で直した点

なし。実装は指示書どおりで、修正の必要は無かった。

早期 return の条件を `nodeTable` の有無で分岐させたのは指示書より丁寧である。
指示書は「`★☆` 除外前のセルの有無で決める」としか書いていなかったが、
nodeTable 不在時は従来の除外後配列による条件を維持しており、縮退経路を変えない制約をより正確に満たしている。

`isExactSameNode` / `isFullSiblingByMaster` / `isFullSiblingByMasterNodeIds` / `isInbreedExcludedHorse` が
残っているのは死にコードではない。それぞれ縮退経路（`isCrossRelated`）と legacy 経路で現役であることを確認した。

### 画面確認

ポート8767 で、事前に ServiceWorker とキャッシュを消してから確認した。
新しい判定コード（`isRefCrossRelated` を含む）が配信されていることを確認したうえで、
アプリ本来の選択ハンドラ `onChangeMain` でダンスインザダーク×エアグルーヴを組んだ。

- `inbreedColorIndexes` = `[6,22]`、Vue の `dispColor` も `[6,22]`（ノーザンダンサー 血量12500）
- Node の測定値と一致
- コンソールエラーは ServiceWorker 登録失敗のみ。フェーズ2aの検収で、
  無改造の旧コピー（ポート8766）でも再現することを対照実験で確認済みの環境要因

工程診断への resolver 配線も確認した。`plan-diagnosis-ui.js:93` が `resolver: this.identityResolver` を渡し、
`plan-diagnosis.js:395,429` が `input.resolver || null` で受けている。`diagnoseBreedingPlan` の呼び出し元は1箇所だけである。

### フェーズ2cへの申し送り

**`resolver` が渡らないとクロスが全滅する。** 指示書で許容した縮退だが、性質として脆い。
`isRefCrossRelated` は resolver が無いと常に false を返すので、将来の改修で第4引数を渡し忘れると
**エラーも警告も出ないままクロスが1件も出なくなる**。現時点で呼び出し元2箇所はどちらも正しく渡している。

フェーズ2cで、`nodeTable` があるのに `resolver` が無い場合に `console.warn` を出すか、
旧 master 判定へ graceful にフォールバックするかを検討すること。
